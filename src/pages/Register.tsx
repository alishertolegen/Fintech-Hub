...
export default function RegisterPage() {
  ...
  return (
    <div className="register-container">
      {/* ДОКУМЕНТАЦИЯ: 
        Динамически добавляем класс 'investor-layout', когда выбрана роль 'investor'.
        Этот класс будет использован в CSS для расширения карточки.
      */}
      <div className={`register-card ${role === 'investor' ? 'investor-layout' : ''}`}>
        ...
        {/* ДОКУМЕНТАЦИЯ: 
          Форме также добавлен класс 'investor-layout' для управления
          внутренней сеткой (grid) полей.
        */}
        <form onSubmit={submit} className={`register-form ${role === 'investor' ? 'investor-layout' : ''}`}>
          
          {/* === Блок 1: Основная информация (Всегда виден) === */}
          ...
          {/* === Блок 2: Выбор роли (Всегда виден) === */}
          <div className="form-group">
          ...
          {/* ДОКУМЕНТАЦИЯ: 
            Это новый контейнер-сетка. 
            - По умолчанию (для founder) он будет в 1 колонку.
            - Когда форма получит класс 'investor-layout', CSS переключит его в 2 колонки.
          */}
          <div className="form-content-split">
            
            {/* === Колонка 1: Общий профиль (Опционально) === */}
          ...
          {/* === Блок 3: Ошибка и Действия (Всегда виден) === */}
          {error && (

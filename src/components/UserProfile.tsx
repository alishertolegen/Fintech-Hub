...
export default function UserProfile() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`http://localhost:8080/api/users/${encodeURIComponent(id)}`, {
          headers,
          signal: abort.signal,
        });
        if (!res.ok) {
  ...
  useEffect(() => {
    if (!user || user.role !== 'investor' || !user.id) return;
    const abort = new AbortController();
    (async () => {
      try {
        const headers: Record<string, string> = {};
  ...
  useEffect(() => {
    if (!user || user.role !== 'founder' || !user.id) return;
    const abort = new AbortController();
    ...
    (async () => {
      try {
        const headers: Record<string, string> = {};
        if (token) headers['Authorization'] = `Bearer ${token}`;
        const res = await fetch('http://localhost:8080/api/startups', { headers, signal: abort.signal });
        if (res.ok) {
          const all: Startup[] = await res.json();
          // Only show public/visible startups for other users
          const theirs = all.filter(s => s.founderId === user.id && s.visibility !== 'private');
          setStartups(theirs);
          const offersAll: Offer[] = [], invAll: Investment[] = [];
          await Promise.all(theirs.map(async s => {
            try {
              const [oR, iR] = await Promise.all([
                fetch(`http://localhost:8080/api/offers?startupId=${s.id}`, { headers, signal: abort.signal }),
                fetch(`http://localhost:8080/api/investments/startup/${s.id}`, { headers, signal: abort.signal }),
              ]);
  ...
  if (error) return (
    <div className="pf-page">
      <div className="pf-state" style={{ flexDirection: 'column', gap: 16 }}>
  ...
  if (!user) return (
    <div className="pf-page"><div className="pf-state">Профиль недоступен.</div></div>
  );

  const phone    = user.phone ?? user.meta?.phone ?? '—';
  const location = user.location ?? user.meta?.location ?? '—';
    ...
    <div className="pf-page">
      <div className="pf-wrap">
        ...
        <div style={{ marginBottom: 18 }}>
          <button className="pf-btn pf-btn-ghost" onClick={() => navigate(-1)}
            ...
            <div className="pf-hero-inner">
              ...
              <div className="pf-hero-contacts">
                {[
                  ...
                  { icon: <Phone size={13} />,   label: 'Телефон',        value: phone },
                  { icon: <MapPin size={13} />,  label: 'Местоположение', value: location },
          ...
          <div className="pf-panel">
            ...
            {isInvestor && (
              <>
                ...
                {(investor?.legalName || investor?.type || investor?.description) && (
                ...
                {/* Investment list */}
                {(loadingInv || investments.length > 0) && (
                  <div className="pf-section">
                    <div className="pf-section-head">
                      <div className="pf-section-label"><Clock size={10} className="icon-blue" />Сделки</div>
                    </div>
                    {loadingInv ? (
            ...
            {isFounder && (
              <>
                {/* Summary stats */}
                {(startups.length > 0 || founderOffers.length > 0) && (
                  <div className="pf-section">
                    <div className="pf-section-head">
                    ...
                    <div className="pf-stats-grid">
                      ...
                      <div className="pf-stat-card">
                        ...
                        <div className="pf-stat-sub">публичных</div>
                      </div>
                      <div className="pf-stat-card">
                ...
                <div className="pf-section">
                  <div className="pf-section-head">
                    <div className="pf-section-label"><Rocket size={10} className="icon-orange" />Стартапы</div>
            ...
            {!isInvestor && !isFounder && (
              <div className="pf-section">
                <div className="pf-empty">
                  <div className="pf-empty-icon"><Users size={16} /></div>

...
export default function EditStartup(): JSX.Element {
  ...
  /* --- Wizard step kept for reuse of Create UI (optional) --- */
    ...
    fetch(`${API}/${encodeURIComponent(id)}`, { credentials: 'include' })
      ...
      .then((data: any) => {
        ...
        if (ms) {
          setWithMetric(true);
          setMetricMrr(ms.mrr ?? '');
          // server legacy field names: users or activeUsers
  ...
  function next() { const err = validateStep(step); if (err) { setError(err); return; } setError(null); setStep(s => Math.min(s+1,3)); }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      // build payload: only include metricsSnapshot if user wants to update metrics
      const payload: any = {
        ...(name ? { name: name.trim() } : {}),
      ...
      if (withMetric) {
        ...
        if (metricOther.trim()) {
          try { ms.other = JSON.parse(metricOther); } catch { throw new Error('Поле "Other" должно быть валидным JSON.'); }
        }
        // Only attach metricsSnapshot if we actually have at least one property (prevents accidental overwrite)
  ...
  if (loading) return <div className="cs-root"><main className="cs-right"><div className="cs-panel">Загрузка…</div></main></div>;
  ...
  return (
    <div className="cs-root">
      ...
      <main className="cs-right">
        ...
        {/* reuse Create UI panels - only step 0..3 used here */}
        {step === 0 && (
          <div className="cs-panel" key="step0">
            ...
            <div className="cs-nav">
              ...
              <div className="cs-nav-right">
                ...
                <button type="button" className="cs-btn cs-btn-next" onClick={next}>Далее <ArrowRight size={16} /></button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="cs-panel" key="step1">
            ...
            <div className="cs-nav">
              <div className="cs-nav-left">
                <button type="button" className="cs-btn cs-btn-ghost" onClick={prev}><ArrowLeft size={16} /> Назад</button>
            ...
            <div className="cs-fields">
              <div className={`cs-metric-toggle ${withMetric ? 'on' : ''}`} onClick={() => setWithMetric(v => !v)}>
                ...
                <div className="cs-toggle-label">
                  <div className="cs-toggle-title"><BarChart2 size={14} style={{display:'inline', marginRight: 6}} />Добавить метрики</div>
              ...
              {withMetric && (
                <div className="cs-metric-fields">
                  ...
                  <div className="cs-metric-row-2">
                    ...
                    <div className="cs-field">
                      <label className="cs-label">{valuationMode === 'pre' ? 'Valuation Pre-money' : 'Valuation Post-money'}<span className="cs-label-req"> *</span></label>
                      {valuationMode === 'pre'
        ...
        {step === 3 && (
          <div className="cs-panel" key="step3">
            ...
            <div className="cs-summary">
              ...
              <div className="cs-summary-block">
                <div className="cs-summary-block-title">Детали<button className="cs-edit-link" onClick={() => setStep(1)}>изменить</button></div>
                <div className="cs-summary-row"><span className="cs-summary-key">Сайт</span><span className="cs-summary-val accent">{website || '—'}</span></div>
                <div className="cs-summary-row"><span className="cs-summary-key">Видимость</span><span className="cs-summary-val">{visibility === 'public' ? '🌍 Публичный' : '🔒 Приватный'}</span></div>
                <div className="cs-summary-row"><span className="cs-summary-key">Модель оценки</span><span className="cs-summary-val purple">{valuationMode.toUpperCase()}-money</span></div>
                <div className="cs-summary-row"><span className="cs-summary-key">Вложений</span><span className="cs-summary-val">{parseAttachments(attachmentsText).length} файл(ов)</span></div>
              </div>

              <div className="cs-summary-block">
                <div className="cs-summary-block-title">Метрики<button className="cs-edit-link" onClick={() => setStep(2)}>изменить</button></div>
                {!withMetric ? (
                  <div className="cs-summary-row"><span className="cs-summary-key">Статус</span><span className="cs-summary-val">Метрики не добавлены</span></div>
                ) : (
                  <>
                    <div className="cs-summary-row"><span className="cs-summary-key">MRR</span><span className="cs-summary-val green">{fmt(metricMrr === '' ? null : Number(metricMrr))}</span></div>
                    <div className="cs-summary-row"><span className="cs-summary-key">Active Users</span><span className="cs-summary-val">{metricActiveUsers !== '' ? Number(metricActiveUsers).toLocaleString() : '—'}</span></div>
                    {metricBurnRate !== '' && (<div className="cs-summary-row"><span className="cs-summary-key">Burn Rate</span><span className="cs-summary-val">{fmt(Number(metricBurnRate))}</span></div>)}
                    <div className="cs-summary-row"><span className="cs-summary-key">Valuation ({valuationMode})</span><span className="cs-summary-val purple">{fmt(valuationMode === 'pre' ? (metricValuationPre !== '' ? Number(metricValuationPre) : null) : (metricValuationPost !== '' ? Number(metricValuationPost) : null))}</span></div>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="cs-nav">
              <div className="cs-nav-left"><button type="button" className="cs-btn cs-btn-ghost" onClick={prev}><ArrowLeft size={16} /> Назад</button><span className="cs-progress">04 / 04</span></div>
              <div className="cs-nav-right">
                <button type="button" className="cs-btn cs-btn-submit" onClick={handleSave} disabled={saving}>{saving ? 'Сохранение…' : <><Check size={16} /> Сохранить</>}</button>
              </div>
            </div>
          </div>
        )}
